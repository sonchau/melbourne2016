...
(function(){
	...
	var each = helpers.each = function(loopable,callback,self){
		...
		formatNumber = helpers.formatNumber = function (num, format) { //(KH) formatNumber 
            if (typeof format === "undefined") { format = this.options.dataPointLabelFormat } //makes the param optional


            if (format == "Currency" || format == "currency" || format == "Money" || format == "money") { //http://stackoverflow.com/questions/149055/how-can-i-format-numbers-as-money-in-javascript
                return num.toFixed(2).replace(/./g, function (c, i, a) {
                    return i > 0 && c !== "." && (a.length - i) % 3 === 0 ? "," + c : c;
                });
            }

            if (format == "group" || format == "Group") { //http://stackoverflow.com/questions/149055/how-can-i-format-numbers-as-money-in-javascript
                var decimalPlacing = (num % 1 != 0 ? 2 : 0)

                return num.toFixed(decimalPlacing).replace(/./g, function (c, i, a) {
                    return i > 0 && c !== "." && (a.length - i) % 3 === 0 ? "," + c : c;
                });
            }
